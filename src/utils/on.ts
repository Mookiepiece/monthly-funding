...
type EventModifiers = 'stop' | 'prevent' | 'self' | 'capture' | 'once' | 'passive';
 ...
 */
const modifierGuards: Record<string, (e: Event, set: Set<string>) => void | boolean> = {
	...
	exact: (e, set) => systemModifiers.some(m => (e as KeyedEvent)[`${m}Key`] && !set.has(m)),
};
