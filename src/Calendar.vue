...
const firstDayOfWeek = computed(() => model.value.localeData().firstDayOfWeek());
...
const useCells = (_today: Ref<Dayjs>) =>
	...
	>(() => {
		...
		const landmark = $month.set('date', Math.min(App.salaryDay, $month.daysInMonth()));
...
<template>
	<section class="Calendar">
		<header class="nav">
			<VButton @click="mav(-1)" @keydown.left.prevent="mav(-1)" @keydown.right.prevent="mav(1)">
		...
		<div class="grid">
			...
			<template v-for="cell in cells" :data-day="cell">
				...
				<div v-else :data-today="cell.day.isSame(App.today, 'date') || undefined">
