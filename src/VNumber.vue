 ...
 */
export const onStepWheel = (el: HTMLElement | SVGElement, cb: (delta: number) => void) => {
	let delta = 0;
...
watchEffect(onCleanup => {
	const $el = el.value;
	if (!$el) return;
	onCleanup(on($el).keydown['ArrowUp'].prevent(() => nav(1)));
	onCleanup(on($el).keydown['ArrowRight'].prevent(() => nav(1)));
	onCleanup(on($el).keydown['ArrowLeft'].prevent(() => nav(-1)));
	onCleanup(on($el).keydown['ArrowDown'].prevent(() => nav(-1)));
});
...
<template>
	<div>
		<input ref="el" v-model="composed" @blur="shadow = null" :name :id :tabindex />
	</div>
</template>
