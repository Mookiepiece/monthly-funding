...
import { ComponentInstance, computed, ref, watchEffect } from 'vue';
...
watchEffect(onCleanup => {
	...
	onCleanup(on($island).mouseenter(throttleBoth(() => (open.value = 1))));
	onCleanup(on($island).mouseleave(throttleBoth(() => (open.value = 0))));
});
watchEffect(onCleanup => {
	const $island: HTMLElement = islandEl.value;
	if (!$island) return;
	...
	if (!open.value) {
		onCleanup(on($island).touchstart(() => (open.value = 1)));
	} else {
		onCleanup(
			on(document).touchstart(e => {
				if (e.target instanceof Element && e.target.closest('button')) return;

				if (e.target instanceof Element && !$island.contains(e.target)) open.value = 0;
			}),
		);
	}
});
